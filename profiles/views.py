    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if unsuccessful_attempts:
                return Response(
                    {
                        "message": "Some student were not added successfully.",
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            for teacher in teachers:
                if not (
                    User.objects.filter(email=teacher[0]).exists()
                    or User.objects.filter(username=teacher[1] + teacher[2])
                ):
                    users.append(
                        {
                            "email": teacher[0],
            ...
            return Response(
                {"success": "Teachers were added successfully"},
                status=status.HTTP_200_OK,
            )
        return Response({"error": "Invalid file"}, status=status.HTTP_400_BAD_REQUEST)
...
class LoginView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        obj = {"user": str(request.user), "auth": str(request.auth)}
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user is not None:
            ...
            obj = {"message": "Login successful", "tokens": tokens}
...
class MyTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not User.objects.filter(email=email).exists():
            return Response(
                data={"error": "Email not found"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if serializer.is_valid():
            ...
            if not (current_user == self.object or current_user.is_superuser):
                raise PermissionError(
                    "You don't have permission to change this user's name."
                )
            ...
            return Response(
                {
                    "success": f"Name successfully changed to {user.first_name} {user.last_name}"
                },
                status=status.HTTP_200_OK,
            )
