...
class Subject(models.Model):
    ...
    teachers = models.ManyToManyField(
        Teacher, related_name="subjects", default=[main_teacher], null=True, blank=True
    )
...
class Chapter(models.Model):
    ...
    def save(self, *args, **kwargs):
        existing_chapters = (
            Chapter.objects.filter(subject=self.subject).distinct().count()
        )
...
class Comment(models.Model):
    ...
    downvotes = models.ManyToManyField(
        User, blank=True, related_name="downvoted_comments"
    )
    attachment = models.FileField(blank=True, null=True, upload_to="forum/")
